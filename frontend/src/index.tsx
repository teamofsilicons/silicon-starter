...
const Icon = (props: { name: string; size?: number }) => {
  const paths: Record<string, string> = {
    ...
    book: 'M4 5a2 2 0 0 1 2-2h14v18H6a2 2 0 0 1-2-2V5Zm0 12h16', plus: 'M12 5v14M5 12h14', copy: 'M9 9h12v12H9V9ZM3 15V3h12', template: 'M4 4h6v6H4V4Zm10 0h6v6h-6V4ZM4 14h6v6H4v-6Zm10 3h6m-3-3v6',
  };
...
function Template(props: { id: string; repository: Repository }) {
  const source = (path = '') => repoUrl(props.id, path ? 'blob' : 'tree', `.starterbase${path ? `/${path}` : ''}`);
  const value = (value: unknown) => typeof value === 'string' && value ? value : JSON.stringify(value, null, 2);
  const ingredients = () => props.repository.files.filter(file => file.path.startsWith('.starterbase/'));
    ...
    <Show when={props.repository.template} fallback={<Show when={!props.repository.template_error}><div class="notice">Template metadata is unavailable. Browse the source ingredients below.</div></Show>}>{recipe => <>
      ...
      <ol class="template-flow">
        ...
        <li><section class="panel template-stage"><header><span>2</span><h3>Resolve answers in order</h3></header><p>Press Enter to accept a resolved default. Saved answers are reused on updates; variables without a prompt resolve silently.</p>
          <ol class="template-questions"><For each={recipe().variables} fallback={<li class="muted">This template has no variables.</li>}>{variable => <li class="template-question" data-variable={variable.name}>
            <div class="question-heading"><code>{variable.name}</code><span class="badge">{variable.type}</span><Show when={!variable.prompt}><span class="muted">No question · resolves silently</span></Show></div>
            <Show when={variable.prompt}><h4>{variable.prompt}</h4></Show>
            <dl><Show when={variable.when}><div><dt>Active when</dt><dd><pre><code>{variable.when}</code></pre></dd></div></Show><Show when={variable.choices != null}><div><dt>Choices</dt><dd><pre><code>{value(variable.choices)}</code></pre></dd></div></Show><div><dt>Default</dt><dd><pre><code>{value(variable.default)}</code></pre></dd></div></dl>
            <Show when={variable.type === 'secret'}><p class="muted">Input is masked in the terminal. The answer is saved locally and available to templates and scripts.</p></Show>
          </li>}</For></ol>
        </section></li>
        <li><section class="panel template-stage"><header><span>3</span><h3>Render files</h3></header><p>Templates use the active answers. Other declared files copy unchanged into a fresh build directory.</p><ul class="template-files"><For each={recipe().files} fallback={<li class="muted">No declared files; build scripts may create outputs.</li>}>{file => <li><a href={source(file.from)}><code>{file.from}</code></a><span aria-label="produces">→</span><code>{file.to}</code></li>}</For></ul></section></li>
        <li><section class="panel template-stage"><header><span>4</span><h3>Run build steps</h3></header><p>Commands run in the recipe directory after rendering. Script output in <code>STARTER_OUTPUT</code> is included in the build.</p><ol class="template-build"><For each={recipe().build} fallback={<li class="muted">No build commands.</li>}>{command => <li><pre><code>{command}</code></pre></li>}</For></ol></section></li>
        <li><section class="panel template-stage"><header><span>5</span><h3>Validate and apply</h3></header><p>Starter validates the generated configuration and merges it with local edits. Answers and the generated baseline are saved after the complete build succeeds. An unresolved update leaves live files intact.</p></section></li>
      </ol>
    </>}</Show>
    <section class="panel template-ingredients"><header><h3>Source ingredients</h3><a href={source()}>Browse .starterbase</a></header><Show when={props.repository.truncated}><p class="notice">This preview is incomplete. Download the Git bundle to see all source ingredients.</p></Show><For each={ingredients()}>{file => <a class="file-row" href={repoUrl(props.id, 'blob', file.path)}><Icon name="file" /><span>{file.path.slice('.starterbase/'.length)}</span><small>{sizeLabel(file)}</small></a>}</For></section>
  </section>;
}
...
function RepositoryPage(props: { id: string; route: Route; session: Session; navigate: (path: string) => void; login: () => void }) {
  ...
  const hasTemplate = () => files().some(file => file.path === '.starterbase/starter.yaml');
  ...
  return <Show when={!starter.error} fallback={<section class="page-width"><a href="/">← Explore starters</a><ErrorMessage error={starter.error} retry={reloadStarter} /></section>}>
    <Show when={starter()} fallback={<div class="page-width loading">Loading starter…</div>}>{item => <>
      ...
      <div class="page-width repository-body"><Show when={notice()}><div class="notice" role="status">{notice()}</div></Show><Show when={actionError()}><ErrorMessage error={actionError()} /></Show>
        ...
        <Show when={tab() === 'template'}><Show when={!repository.error} fallback={<ErrorMessage error={repository.error} retry={reloadFiles} />}><Show when={!repository.loading} fallback={<div class="loading panel">Reading template…</div>}><Show when={hasTemplate()} fallback={<div class="empty panel"><h2>No template recipe</h2><p>{repository()?.truncated ? 'The repository preview is incomplete. Download the Git bundle to check for a recipe.' : 'This starter uses its files directly, without a .starterbase/starter.yaml recipe.'}</p><a href={repoUrl(props.id)}>Browse repository files</a></div>}><Template id={props.id} repository={repository()!} /></Show></Show></Show></Show>
        ...
        <Show when={!['code', 'template', 'architecture', 'releases', 'discussions'].includes(tab())}><div class="empty panel"><h2>Page not found</h2><a href={repoUrl(props.id)}>View code</a></div></Show>
      </div>
    </>}</Show>
  </Show>;
}
